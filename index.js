...
async function triggerBackendSync() {
  try {
    ...
    if (data.ok) {
      console.log('[sync] ✅ sitemap.xml pushed to GitHub (' + data.bonusCount + ' bonuses) at ' + data.timestamp);
    } else {
      console.error('[sync] ❌ backend error:', data.error);
    }
  } catch(e) {
    console.error('[sync] ❌ could not reach backend — is the Replit server running?', e.message);
  }
}
