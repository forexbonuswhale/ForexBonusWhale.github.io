...
// ════════════════════════════════════════════════════════════
//  BACKEND SYNC — update this URL after deploying the backend
// ════════════════════════════════════════════════════════════
const BACKEND_URL = 'https://7647b901-94b6-4c92-a85e-2fb43286b486-00-1pru9fmv5hk50.sisko.replit.dev/api';

async function triggerBackendSync() {
  try {
    const res = await fetch(`${BACKEND_URL}/github-sync`, { method: 'POST' });
    const data = await res.json();
    if (data.ok) {
      console.log('[sync] sitemap.xml pushed to GitHub (' + data.bonusCount + ' bonuses)');
    } else {
      console.warn('[sync] backend returned error:', data.error);
    }
  } catch(e) {
    console.warn('[sync] could not reach backend:', e.message);
  }
}
...
document.addEventListener('click', function(e) {
  ...
  if      (action==='nav')         { navigate(btn.dataset.sec); }
    ...
    if(confirm('Delete this bonus?')){
      deleteBonus(btn.dataset.id).then(()=>{ triggerBackendSync(); refreshBonuses(); });
    }
  }
});
