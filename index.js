...
const I18N = {
  ...
  fr: {
    navNoDeposit:'Bonus sans dépôt', navDeposit:'Bonus de dépôt',
    ...
    minDep:'Dépôt min. :', statusLabel:'Statut :', verifiedLabel:'Vérifié :',
    yes:'Oui', no:'Non', adminPanel:'Panneau Admin', allBonuses:'TOUS LES BONUS',
    addBonus:'AJOUTER UN BONUS', editBonus:'MODIFIER LE BONUS',
    morFrom:'PLUS DE', footerTag:'Votre guide indépendant des meilleurs bonus Forex. Toujours vérifiés, toujours à jour.',
    contact:'Questions ?',
  },
  de: {
...
// ── Star rating widget (CSS-only hover — never replaces DOM on hover) ─────────
// Stars rendered 5→1 with flex-direction:row-reverse so CSS ~ sibling selector
// highlights all stars ≤ the hovered one without any JS DOM mutation.
...
async function submitReview(bonusId) {
  ...
  await postReview(bonusId,{name,rating,comment,countryCode:S.userCountry});
  ...
  S.reviews.unshift({id:'new_'+Date.now(),name,rating,comment,countryCode:S.userCountry,createdAt:Date.now()});
...
document.addEventListener('click', function(e) {
  ...
  if      (action==='nav')         { navigate(btn.dataset.sec); }
    ...
    // Toggle sel class on existing buttons — never replaces DOM elements
...
//  INIT
// ════════════════════════════════════════════════════════════
// Detect user country for review flags (best-effort, silent on fail)
