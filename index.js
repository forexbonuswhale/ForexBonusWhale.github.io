...
function getGhPat() { try{return localStorage.getItem('fbw_gh_pat')||'';}catch(e){return '';} }
function setGhPat(v) { try{if(v)localStorage.setItem('fbw_gh_pat',v);else localStorage.removeItem('fbw_gh_pat');}catch(e){} }

// ── Client-side sitemap builder + instant push to GitHub ────
function buildSitemapXml(bonuses) {
  const SU  = 'https://forexbonuswhale.github.io';
  const day = new Date().toISOString().split('T')[0];
  const statics = [
    {loc:SU+'/',           lastmod:day,changefreq:'daily', priority:'1.0'},
    {loc:SU+'/deposit',    lastmod:day,changefreq:'daily', priority:'0.9'},
    {loc:SU+'/no-deposit', lastmod:day,changefreq:'daily', priority:'0.9'},
    {loc:SU+'/contest',    lastmod:day,changefreq:'daily', priority:'0.9'},
  ];
  const bpages = bonuses.map(b=>{
    let lm=day; try{if(b.createdAt)lm=new Date(+b.createdAt).toISOString().split('T')[0];}catch(e){}
    return {loc:`${SU}/bonus/${b.slug||b.id}`,lastmod:lm,changefreq:'weekly',priority:'0.8'};
  });
  return '<?xml version="1.0" encoding="UTF-8"?>\n'+
    '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'+
    [...statics,...bpages].map(u=>
      `  <url>\n    <loc>${u.loc}</loc>\n    <lastmod>${u.lastmod}</lastmod>\n    <changefreq>${u.changefreq}</changefreq>\n    <priority>${u.priority}</priority>\n  </url>`
    ).join('\n')+'\n</urlset>';
}
async function tryUpdateSitemap() {
  const pat=getGhPat(); if(!pat) return;
  try {
    const xml    = buildSitemapXml(S.bonuses);
    const apiUrl = 'https://api.github.com/repos/forexbonuswhale/forexbonuswhale.github.io/contents/sitemap.xml';
    const hdrs   = {'Authorization':`Bearer ${pat}`,'Accept':'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','Content-Type':'application/json'};
    let sha;
    const gr=await fetch(`${apiUrl}?ref=main`,{headers:hdrs});
    if(gr.ok){sha=(await gr.json()).sha;}
    const body={message:`chore: regenerate sitemap.xml [skip ci] — ${new Date().toISOString()}`,content:btoa(xml),branch:'main'};
    if(sha)body.sha=sha;
    const pr=await fetch(apiUrl,{method:'PUT',headers:hdrs,body:JSON.stringify(body)});
    if(!pr.ok){const e=await pr.text();console.warn('Sitemap push failed:',e);}
    else{console.log('✅ Sitemap updated instantly');}
  } catch(e){console.warn('Sitemap update error:',e.message);}
}

// ── Slug helper ─────────────────────────────────────────────
function slugify(brokerName, amount, type) {
  return [brokerName, amount, type].filter(Boolean).join(' ')
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, '')
    .trim()
    .replace(/\s+/g, '-')
...
function viewDetails(id) {
  ...
  render();
  const bonus=S.bonuses.find(b=>(b.slug||b.id)===id);
  const bonusFbId=bonus?bonus.id:id;
  fetchAndShowReviews(bonusFbId);
  if (bonus) incrementViews(bonusFbId,+bonus.views||0).then(v=>{if(bonus)bonus.views=v;});
...
function showReviews() {
  ...
  const _selBonus=S.bonuses.find(b=>(b.slug||b.id)===S.selectedId);
  const _selFbId=_selBonus?_selBonus.id:S.selectedId;
  const myId=getMyReviewId(_selFbId);
  let html=pageRevs.map(r=>{
    ...
    return `<div class="review-item">
      <div class="review-top">
        ...
        ${isOwn?`<button class="rev-del-btn" data-action="del-review" data-bid="${esc(_selFbId)}" data-rid="${esc(r.id)}" title="Delete your review">✕ Delete</button>`:''}
      </div>
...
async function handleAdminSubmit(e) {
  ...
  const _brokerName=(fd.get('brokerName')||'').trim();
  const _amount=(fd.get('amount')||'').trim();
  const _type=fd.get('type')||'deposit';
  const _slug=(fd.get('slug')||'').trim()||slugify(_brokerName,_amount,_type);
  const data={
...
<div class="broker-card">
  ...
  <button class="view-btn" data-action="view" data-id="${esc(b.slug||b.id)}">${esc(t('viewDetails'))}</button>
</div>`;
}
...
<div class="page-content" style="padding-top:0">
  ...
  <div class="detail-card">
    ...
    <p style="text-align:center;font-size:11px;color:var(--text3);margin-top:7px;line-height:1.5;padding:0 4px">↗ This button opens the broker's official website in a new tab.<br>ForexBonusWhale is an independent listing site and is not responsible for third-party content.</p>
    <p style="text-align:center;font-size:10px;color:var(--text3);margin-top:4px;opacity:.75">⚠️ Trading Forex involves significant risk of loss. This is not financial advice.</p>
  </div>
  ...
  <div class="detail-sec-card">
    ...
    <div class="related-list">
      ${related.map(rb=>{
        ...
        return `
      <button class="related-btn" data-action="view" data-id="${esc(rb.slug||rb.id)}">
        ${rb.brokerImageUrl
  ...
  <form id="admin-form" onsubmit="handleAdminSubmit(event)">
    <div class="form-grid">
      ...
      <div class="form-field" style="grid-column:1/-1">
        <label class="form-label">URL Slug (auto-generated from broker name + amount + type if left empty)</label>
        <input class="form-input" name="slug" value="${esc(b.slug||'')}" placeholder="e.g. xm-30-no-deposit-bonus"/>
      </div>
...
function renderAdmin() {
  const patSaved=!!getGhPat();
  return `
<div class="page-content">
  <div class="admin-title">${esc(t('adminPanel'))}</div>
  <div class="detail-sec-card" style="margin-bottom:16px">
    <div class="dsec-title">⚡ Instant Sitemap</div>
    ${patSaved
      ? `<p style="font-size:13px;color:var(--text2);margin:0 0 8px">✓ Token saved — sitemap updates automatically every time you add, edit, or delete a bonus.</p>`
      : `<p style="font-size:13px;color:var(--text2);margin:0 0 8px">Paste your GitHub Personal Access Token once to enable instant sitemap updates the moment you save a bonus.</p>`
    }
    <div style="display:flex;gap:8px;align-items:center">
      <input type="password" id="gh-pat-input" class="form-input" style="flex:1;margin:0"
        value="${esc(getGhPat())}" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx"/>
      <button type="button" class="submit-btn" style="flex-shrink:0;padding:8px 14px;margin:0" data-action="save-gh-pat">${patSaved?'Update':'Save'}</button>
    </div>
    <p style="font-size:11px;color:var(--text3);margin:6px 0 0">Token is stored only in your browser — never in the code. Needs <b>Contents: Read &amp; Write</b> permission on this repo.</p>
  </div>
  ${renderAdminForm()}
...
function render() {
  ...
  const bonus = S.selectedId ? S.bonuses.find(b=>(b.slug||b.id)===S.selectedId) : null;
  let mainHTML = '';
...
document.addEventListener('click', function(e) {
  ...
  if      (action==='nav')         { navigate(btn.dataset.sec); }
    ...
    if(claimUrl){
      ...
      toast.style.cssText='position:fixed;bottom:28px;left:50%;transform:translateX(-50%);background:#1d4ed8;color:#fff;padding:11px 22px;border-radius:12px;font-size:13px;font-weight:600;z-index:9999;box-shadow:0 4px 18px rgba(0,0,0,.35);text-align:center;line-height:1.5;max-width:320px;width:90%';
      toast.innerHTML='↗ Opening broker\'s official website in a new tab&hellip;<br><span style="font-size:11px;font-weight:400;opacity:.85">ForexBonusWhale is not affiliated with this broker.</span>';
      document.body.appendChild(toast);
      setTimeout(()=>{ toast.remove(); window.open(claimUrl,'_blank','noopener,noreferrer'); },1400);
    }
  }
  ...
  else if (action==='save-gh-pat')    {
    const pi=document.getElementById('gh-pat-input');
    if(pi){ setGhPat(pi.value.trim()); render(); }
  }
  else if (action==='admin-del')      {
    if(confirm('Delete this bonus?')){
      deleteBonus(btn.dataset.id).then(async()=>{ await refreshBonuses(); tryUpdateSitemap(); });
    }
  }
});
...
window.addEventListener('popstate', ()=>{
  ...
  render();
  if(S.selectedId){
    const bonus=S.bonuses.find(b=>(b.slug||b.id)===S.selectedId);
    if(bonus){
      fetchAndShowReviews(bonus.id);
      incrementViews(bonus.id,+bonus.views||0).then(v=>{if(bonus)bonus.views=v;});
    }
  }
});
