...
const I18N = {
  ...
  ja: {
    ...
    morFrom:'他のボーナス', footerTag:'最高のForexブローカーボーナスのための独立したガイド。常に検証済み、常に最新。',
    contact:'お問い合わせ',
  },
};
...
function interactiveStars(cur, hov) {
  return [1,2,3,4,5].map(i=>`
    <button class="star-btn" data-action="rev-star" data-val="${i}"
...
function viewDetails(id) {
  ...
  // Translate detail content if not English
  if (S.lang !== 'en' && bonus) setTimeout(()=>translateDetailFields(bonus), 100);
}
function goBack() {
  S.selectedId=null;
  const map={'no-deposit':'',deposit:'/deposit',contest:'/contest',about:'/about',admin:'/admin'};
  window.location.hash=map[S.section]||'';
  render();
...
async function submitReview(bonusId) {
  ...
  await postReview(bonusId,{name,rating,comment});
  await updateBonus(bonusId,{rating:newR,reviews:newN});
  bonus.rating=newR; bonus.reviews=newN;
  S.revSubmitted=true; S.revRating=0;
  S.reviews.unshift({id:'new_'+Date.now(),name,rating,comment,createdAt:Date.now()});
...
// ════════════════════════════════════════════════════════════
//  RENDER: HEADER  (realistic whale logo)
// ════════════════════════════════════════════════════════════
...
<header class="header">
  <div class="header-inner">
    <button class="logo-btn" data-action="nav" data-sec="no-deposit">
...
// ════════════════════════════════════════════════════════════
//  RENDER: MOBILE MENU  (nav-only, no stats)
// ════════════════════════════════════════════════════════════
const NAV_ITEMS = [
...
<nav class="mobile-menu">
  ${NAV_ITEMS.map(item=>`
    <button class="menu-item${S.section===item.id&&!S.selectedId?' menu-item-active':''}"
      data-action="nav" data-sec="${item.id}">${esc(t(item.key))}</button>`).join('')}
</nav>`;
}

// ════════════════════════════════════════════════════════════
//  RENDER: STATS BAR  (short labels, no truncation)
// ════════════════════════════════════════════════════════════
...
<div class="section-dropdown">
  ${['no-deposit','deposit','contest'].map(s=>`
...
<div class="page-content">
  ...
  <div class="sort-row">
    <button class="sort-btn" data-action="sort-toggle">
      ${esc(sortLabels[S.sort]||sortLabels.popular)} ${chevSVG(S.sortOpen)}
    </button>
    ${sortDrop}
  </div>

  <div class="results-count">${filtered.length} ${filtered.length!==1?t('results'):t('result')}</div>
...
<div class="rev-form">
  ...
  <div class="form-field">
    ...
    <div id="rev-stars" style="display:flex;gap:4px">${interactiveStars(S.revRating,S.revHover)}</div>
  </div>
  <div class="form-field">
  ...
  ${S.revSubmitted
    ? `<div class="submit-success">${esc(t('reviewSubmitted'))}</div>`
    : `<button class="submit-review-btn" data-action="submit-review" data-bid="${esc(bonusId)}">
...
function renderDetail(b) {
  ...
  const related = S.bonuses.filter(x=>x.brokerId===b.brokerId&&x.id!==b.id);
  const isActive = statusLC(b) === 'active';
...
<div class="page-content" style="padding-top:0">
  ...
  <div class="detail-card">
    ...
    <a href="${esc(b.claimUrl||'#')}" target="_blank" rel="noopener noreferrer" class="claim-btn">
  ...
  <div class="detail-sec-card">
    ...
    <div class="related-list">
      ...
      <button class="related-btn" data-action="view" data-id="${esc(rb.id)}">
        <span class="related-type">${typeMeta(rb.type).label}</span>
        <span class="related-title">${esc(rb.title)}</span>
...
<div class="page-content">
  <div class="detail-sec-card" style="margin-top:14px">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px">
      <img src="favicon.svg" width="34" height="34" alt="whale"/>
      <span class="logo-text" style="font-size:18px">ForexBonusWhale</span>
    </div>
    <p class="about-lead">Your independent guide to the best Forex broker bonuses. Always verified, always up to date.</p>
    <h2 class="about-h">Our Mission</h2>
    <p class="about-body">ForexBonusWhale curates and verifies Forex trading bonuses — no-deposit bonuses, deposit matches, cashback offers, and trading contests — from regulated, trusted brokers worldwide. We do the research so you can focus on trading.</p>
    <h2 class="about-h">How We Work</h2>
    <p class="about-body">Our team manually reviews each broker and bonus listing. We verify regulatory status, check withdrawal conditions, and confirm bonus availability before publishing. Listings are updated regularly.</p>
    <div class="disclaimer-box">
      <div class="disclaimer-hdr">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
        Important Risk Disclosure &amp; Legal Disclaimer
      </div>
      <p><strong>Trading Forex and CFDs involves significant risk of loss and is not suitable for all investors.</strong> Leverage can work against you. You may lose more than your initial investment. Past performance is not indicative of future results.</p>
      <p style="margin-top:10px"><strong>ForexBonusWhale is an independent information service only.</strong> We do not provide financial, investment, legal, or tax advice. Nothing on this website constitutes a solicitation or recommendation to buy or sell any financial instrument.</p>
      <p style="margin-top:10px"><strong>No Liability:</strong> We expressly disclaim all liability for any financial losses arising from your use of information on this site. All bonus information is provided "as-is". Always verify terms directly with the broker before taking any action.</p>
      <p style="margin-top:10px"><strong>Duplicate Accounts:</strong> We are not responsible for any bans or penalties if a broker determines you have created duplicate accounts to claim the same bonus. Always read each broker's full terms before participating.</p>
      <p style="margin-top:10px;font-style:italic;opacity:.85">By using this website, you acknowledge that you have read and agreed to this disclaimer in its entirety.</p>
    </div>
    <h2 class="about-h">Contact Us</h2>
  ...
  <form id="admin-form" onsubmit="handleAdminSubmit(event)">
    <div class="form-grid">
      ...
      <div class="form-field" style="grid-column:1/-1">
        <label class="form-label">Broker Logo URL</label>
        <input class="form-input" name="brokerImageUrl" value="${esc(b.brokerImageUrl||'')}" placeholder="https://example.com/logo.png" oninput="previewLogo(this.value)"/>
        <div id="logo-preview" style="margin-top:6px">
      ...
      <div class="form-field">
        ...
        <select class="form-input" name="type">
          <option value="no-deposit"${(b.type||'no-deposit')==='no-deposit'?' selected':''}>No Deposit</option>
          <option value="deposit"${b.type==='deposit'?' selected':''}>Deposit</option>
      ...
      <div class="form-field">
        ...
        <select class="form-input" name="status">
          <option value="active"${(statusLC(b)||'active')==='active'?' selected':''}>Active</option>
          <option value="inactive"${statusLC(b)==='inactive'?' selected':''}>Inactive</option>
        </select>
      </div>
      <div class="form-field">
      ...
      <div class="form-field">
        <label class="form-label">Bonus Image URL</label>
        <input class="form-input" name="imageUrl" value="${esc(b.imageUrl||'')}"/>
      </div>
...
// ════════════════════════════════════════════════════════════
//  RENDER: FOOTER  (language flags adjacent to Quick Links)
// ════════════════════════════════════════════════════════════
...
<footer class="footer">
  <div class="footer-inner">
    <button class="logo-btn" style="margin-bottom:10px" data-action="nav" data-sec="no-deposit">
    ...
    <div class="footer-bottom-row">
      <div class="footer-links">
        ...
        ${NAV_ITEMS.map(item=>`<button class="footer-link" data-action="nav" data-sec="${item.id}">${esc(t(item.key))}</button>`).join('')}
      </div>
    ...
    <p class="footer-contact">${esc(t('contact'))} <a href="mailto:forexbonuswhale@gmail.com" class="link-email">forexbonuswhale@gmail.com</a></p>
...
function render() {
  ...
  if (S.selectedId) {
    showReviews();
    // After render, translate detail content if not English
    if (S.lang !== 'en' && bonus) translateDetailFields(bonus);
  }
  // Translate card titles if not English
...
document.addEventListener('click', function(e) {
  ...
  else if (action==='filter')         { S.filter=btn.dataset.val; S.sortOpen=false; S.secOpen=false; render(); }
  else if (action==='sort-toggle')    { S.sortOpen=!S.sortOpen; S.secOpen=false; render(); }
  else if (action==='sort-set')       { S.sort=btn.dataset.val; S.sortOpen=false; render(); }
  else if (action==='sec-toggle')     { S.secOpen=!S.secOpen; S.sortOpen=false; render(); }
  else if (action==='rev-star')       {
    S.revRating=+btn.dataset.val; S.revHover=0;
    const el=document.getElementById('rev-stars');
    if(el) el.innerHTML=interactiveStars(S.revRating,0);
  }
