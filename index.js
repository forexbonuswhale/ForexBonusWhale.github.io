...
function navigate(sec) {
  ...
  const newHash = '#' + (HASH_MAP[sec] || '/deposit');
...
function goBack() {
  S.selectedId=null;
  const newHash = '#' + (HASH_MAP[S.section] || '/deposit');
  history.pushState(null, '', newHash);
  render();
...
<nav class="mobile-menu">
  ...
  <div class="menu-stats-row">
    <div class="menu-stat-item">
      <div class="menu-stat-icon stat-blue"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg></div>
      <div><div class="menu-stat-lbl">${esc(t('statBonuses'))}</div><div class="menu-stat-val">${total}</div></div>
    </div>
    <div class="menu-stat-item">
      <div class="menu-stat-icon stat-green"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></div>
      <div><div class="menu-stat-lbl">${esc(t('statActive'))}</div><div class="menu-stat-val">${active}</div></div>
    </div>
    <div class="menu-stat-item">
      <div class="menu-stat-icon stat-sky"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>
      <div><div class="menu-stat-lbl">${esc(t('statReviews'))}</div><div class="menu-stat-val">${fmt(reviews)}</div></div>
    </div>
  </div>
  <div class="menu-search-wrap">
    <div class="menu-search-inner">
      ...
      <input class="menu-search-input" placeholder="${esc(t('searchPlaceholder'))}"
        value="${esc(S.search)}" oninput="onMenuSearch(this.value)"/>
    </div>
  </div>
</nav>`;
}
...
<div class="page-content">
  <div class="top-nav-row">
    ${NAV_ITEMS.map(item=>`
    <button class="top-nav-pill${S.section===item.id?' top-nav-pill-active':''}"
      data-action="nav" data-sec="${item.id}">
      <span class="top-nav-pill-icon">${NAV_ICONS[item.id]||''}</span>
      ${esc(t(item.key))}
    </button>`).join('')}
  </div>
...
// ════════════════════════════════════════════════════════════
//  HASH / POPSTATE ROUTING
// ════════════════════════════════════════════════════════════
function parseHash() {
  const h = window.location.hash.replace('#','');
  if      (h.startsWith('/bonus/')) { S.selectedId=h.replace('/bonus/',''); }
  else if (h==='/deposit' || h==='' || h==='/') { S.section='deposit';    S.selectedId=null; }
  else if (h==='/no-deposit')       { S.section='no-deposit'; S.selectedId=null; }
