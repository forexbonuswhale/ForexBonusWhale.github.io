...
function sortBrokers(type, sort) {
    currentSort[type] = sort;
...
function filterStatus(type, filter) {
...
function showSection(id) {
    ...
    document.getElementById('broker-details').classList.add('hidden-section');
...
function goBack() {
    document.getElementById('broker-details').classList.add('hidden-section');
    document.getElementById(lastSection).classList.remove('hidden-section');
    window.scrollTo(0, 0);
}

function openBroker(type, broker) {
    document.querySelectorAll('.content-section').forEach(el => el.classList.add('hidden-section'));
    document.getElementById('broker-details').classList.remove('hidden-section');
    
    document.getElementById('detail-name').textContent = broker.name;
    document.getElementById('detail-bonus').textContent = broker.bonus;
    document.getElementById('detail-status').textContent = broker.status.toUpperCase();
    document.getElementById('detail-rating').textContent = broker.rating + '/5';
    document.getElementById('detail-reviews').textContent = broker.reviews;
    document.getElementById('detail-date').textContent = broker.date;
    document.getElementById('detail-desc').textContent = broker.description;
    document.getElementById('detail-link').href = broker.link;
    document.getElementById('review-broker-name').textContent = broker.name;
    
    const statusEl = document.getElementById('detail-badges').querySelector('span');
    if (broker.status === 'active') {
        statusEl.className = 'badge-active px-3 py-1 rounded-lg text-sm font-bold';
        statusEl.textContent = '✓ Active';
    } else {
        statusEl.className = 'badge-inactive px-3 py-1 rounded-lg text-sm font-bold';
        statusEl.textContent = '✗ Inactive';
    }
    
    renderReviews(broker.name);
    window.scrollTo(0, 0);
}

function renderReviews(brokerName) {
    const brokerReviews = reviews[brokerName] || [];
    const list = document.getElementById('reviews-list');
    
    if (brokerReviews.length === 0) {
        list.innerHTML = '<p class="text-slate-500 text-center py-8">No reviews yet. Be the first to review!</p>';
        return;
    }
    
    list.innerHTML = brokerReviews.map(r => `
        <div class="bg-slate-900 p-4 rounded-lg mb-4 border-l-4 border-yellow-500">
...
function submitReview(e) {
    e.preventDefault();
    const form = e.target;
    const brokerName = document.getElementById('review-broker-name').textContent;
    const name = form.querySelector('input[type="text"]').value;
    const rating = parseInt(form.querySelector('select').value);
    const comment = form.querySelector('textarea').value;
    
    if (!reviews[brokerName]) reviews[brokerName] = [];
    reviews[brokerName].unshift({ name, rating, comment });
    saveReviews();
    
    form.reset();
    renderReviews(brokerName);
